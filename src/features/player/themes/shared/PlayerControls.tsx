...
export function PlayerControls(props:Props) {
  ...
  return <div className="full-player__transport pt-transport">
    ...
    <button id="player-btn-play-pause" type="button" className="full-player__play" aria-label={props.loading?'正在加载试听':props.isPlaying?'暂停':'播放'} disabled={props.loading} onClick={()=>invoke(props.onTogglePlay)}>{props.isPlaying?<Pause size={23}/>:<Play size={23}/>}</button>
