...
export function PlayerControls(props:Props) {
  ...
  return <div className="full-player__transport pt-transport">
    ...
    <button id="player-btn-play-pause" type="button" className="full-player__play" data-playing={props.isPlaying} data-loading={props.loading} aria-label={props.loading?'正在查找可靠音源':props.isPlaying?'暂停':'播放'} disabled={props.loading} onClick={()=>invoke(props.onTogglePlay)}>
      {props.artwork && <img src={props.artwork} alt="" draggable={false}/>}<span aria-hidden="true">{props.loading?<i className="player-loading-ring"/>:props.isPlaying?<Pause size={20} fill="currentColor"/>:<Play size={20} fill="currentColor"/>}</span>
    </button>
