...
export function PlayerThemeSelector({preference}:{preference:PlayerThemePreference}) {
  return <div className="pt-selector"><div className="pt-selector__options" role="group" aria-label="播放器样式">
    {Object.values(playerThemeRegistry).map(theme=><button className="pt-selector__option" type="button" key={theme.id} aria-pressed={theme.id===preference.themeId} onClick={()=>preference.setTheme(theme.id)} style={theme.tokens}>
