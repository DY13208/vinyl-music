...
export function CollectionActionMenu({open,onClose,filters,preference,onAddVinyl}:{open:boolean;onClose:()=>void;filters:CollectionBrowseState;preference:CollectionThemeState;onAddVinyl:()=>void}) {
  ...
  return <dialog ref={dialog} className="ct-action-menu" aria-labelledby="collection-actions-title" onCancel={onClose} onClose={onClose} onClick={event=>{if(event.target===event.currentTarget)onClose();}}>
    <div className="ct-action-menu__content">
      <header><h2 id="collection-actions-title">唱片架选项</h2><button type="button" aria-label="关闭唱片架选项" onClick={onClose}><X size={18}/></button></header>
      <button type="button" className="ct-action-menu__add" onClick={()=>{onClose();onAddVinyl();}}><Plus size={17}/>新增唱片</button>
      <label>排序方式<select aria-label="排序方式" value={filters.sort} onChange={event=>filters.setSort(event.target.value as SortOption)}><option value="recent">最近入架</option><option value="artist">艺术家</option><option value="year">发行年份</option></select></label>
      ...
      <label>唱片架页主题<select aria-label="唱片架页主题" value={preference.themeId} onChange={event=>preference.setTheme(event.target.value as CollectionThemeId)}>{collectionThemes.map(theme=><option key={theme.id} value={theme.id}>{theme.name}</option>)}</select></label>
