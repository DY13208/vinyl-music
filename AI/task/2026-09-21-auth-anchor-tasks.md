...
| 状态 | 事实 |
| --- | --- |
| 已实现 | 自建 username/email、Argon2id、PostgreSQL、Redis opaque session、CSRF、SMTP recovery 适配和 React 接入。 |
| 已验证 | 隔离 PostgreSQL 18、Redis 7、内存邮件 fake、HTTP 集成测试、Playwright 两条主流程、类型检查、单测和构建均通过。 |
| 仅设计 | 真实 SMTP 投递与收信人工验证。 |
| 待实现 | 无；真实 SMTP 项按无凭据规则保持 NOT_RUN。 |
...
| [BC-AUTH-01](#bc-auth-01) | 数据与依赖 | [PostgreSQL](../plan/2026-09-21-login-logout-minimal-plan.md#postgresql) | DONE / PASS / MET |
| [BC-AUTH-02](#bc-auth-02) | 会话、改密与邮件找回 API | [后端设计](../plan/2026-09-21-login-logout-minimal-plan.md#3-最小后端设计) | DONE / PASS / MET |
...
| [BC-AUTH-05](#bc-auth-05) | 闭环验收 | [完成标准](../plan/2026-09-21-login-logout-minimal-plan.md#7-完成标准与风险) | DONE / PASS / MET |

共 6 个锚点，已达标 6；实现/验证/验收分别记录为 TODO 或 DONE、NOT_RUN 或 PASS/FAIL、NOT_MET 或 MET。正文状态、总表和末尾证据记录一起更新。
...
| BC-AUTH-01 | `migrations/001_auth.sql`、`scripts/db-migrate.mjs`、`scripts/test-db.mjs`、`tests/integration/auth-db.test.ts`、`.env.example`、`package.json`/`package-lock.json` | PG18 空库 `npm run db:migrate`（0）；`npm run test:db`（0）；`git diff --check`（0） | 隔离 PG18 空库前滚成功；唯一约束、事务回滚、token 摘要/单次消费和有效期测试通过 | 真实生产数据库未迁移，符合本任务不迁移已有业务库边界 | 2026-09-21 14:30 UTC |
| BC-AUTH-02 | `src/server/auth.ts`、`server.ts`、`tests/integration/auth-http.test.ts`、`package.json`（`test:auth`） | 隔离 PG18/Redis7 `npm run test:auth`（0）；`npm run lint`（0）；`npm test`（0）；`npm run build`（0） | 七接口、opaque Cookie、CSRF/Origin、session_version、Argon2id、Redis idle/absolute expiry、并发前置条件和 SMTP fake HTTP 流程通过 | 真实 SMTP 投递未运行 | 2026-09-21 14:30 UTC |
...
| BC-AUTH-05 | `tests/e2e/auth.spec.ts`、`tests/fixtures/auth-server.ts`、`playwright.config.ts`、认证 helper、`src/platform/auth/WebAuthAdapter.ts` | 隔离 PG18/Redis7 `npm run lint`（0）、`npm run test:db`（0）、`npm run test:auth`（0）、`npm test`（0）、`npm run build`（0）、`npm run test:e2e -- tests/e2e/auth.spec.ts`（0，2 passed） | 两条浏览器主流程在真实 PG18/Redis7 + 内存 SMTP fake 上通过；旧 Session 失效、改密、找回重放、登出和刷新恢复均有 HTTP/E2E 证据 | 没有 SMTP 凭据，真实发送/收信项按标准记为 NOT_RUN | 2026-09-21 14:30 UTC |
